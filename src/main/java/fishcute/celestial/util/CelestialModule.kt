...
val module = Module("fishcute/celestial",
    ...
    FunctionList(hashMapOf(
        "print" to Function({arr -> Util.print(
            arr[0] as Double)
        }, 1),
        ...
        "inArea" to Function({arr ->
            if (Util.isInArea(arr[0].toDouble(), arr[1].toDouble(), arr[2].toDouble(), arr[3].toDouble(), arr[4].toDouble(), arr[5].toDouble()))
                1.0 else 0.0;
        }, 6),
        "distanceToArea" to Function({arr ->
            Util.distanceToArea(VMinecraftInstance.getPlayerX(), VMinecraftInstance.getPlayerY(), VMinecraftInstance.getPlayerZ(),
                arr[0].toDouble(), arr[1].toDouble(), arr[2].toDouble(), arr[3].toDouble(), arr[4].toDouble(), arr[5].toDouble())
        }, 6),
